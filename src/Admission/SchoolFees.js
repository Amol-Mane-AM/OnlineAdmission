import React from 'react';
import './SchoolFees.css';

function SchoolFees() {
  return (
    <div className="hero-section">
      <div className="container hero-section fees-container">
        <div className="fees-header">
          <h3>School Fees Structure</h3>
          <div className="fees-info-box">
            <p><strong>ACCOUNTS – Fee Counter Timings:</strong></p>
            <p>🕒 8:00 a.m. to 1:00 p.m. (Monday to Thursday)</p>
            <p>🕒 8:30 a.m. to 11:30 a.m. (Saturday)</p>
          </div>
          <div className="registration-fee">
            Registration FEE – 500 Rs.
          </div>
        </div>

        <h5 className="section-title">Tuition Fees</h5>
        <div className="table-container">
          <table className="custom-table text-center">
            <thead>
              <tr>
                <th>Grade</th>
                <th>Term 1<br /><small>(Incl. of Books)</small></th>
                <th>Term 2<br /><small>Tuition Fee</small></th>
                <th>Term 3<br /><small>Tuition Fee</small></th>
                <th>Annual Fee</th>
              </tr>
            </thead>
            <tbody>
              <tr><td>FS1</td><td>3,662</td><td>2,462</td><td>2,462</td><td>8,586</td></tr>
              <tr><td>FS2</td><td>3,804</td><td>2,462</td><td>2,462</td><td>8,728</td></tr>
              <tr><td>Year 1</td><td>3,912</td><td>2,463</td><td>2,463</td><td>8,838</td></tr>
              <tr><td>Year 2</td><td>4,671</td><td>2,676</td><td>2,676</td><td>10,023</td></tr>
              <tr><td>Year 3</td><td>4,658</td><td>2,670</td><td>2,670</td><td>9,998</td></tr>
              <tr><td>Year 4</td><td>4,658</td><td>2,670</td><td>2,670</td><td>9,998</td></tr>
              <tr><td>Year 5</td><td>4,767</td><td>2,676</td><td>2,676</td><td>10,119</td></tr>
              <tr><td>Year 6</td><td>5,455</td><td>3,191</td><td>3,191</td><td>11,837</td></tr>
              <tr><td>Year 7</td><td>5,545</td><td>3,184</td><td>3,184</td><td>11,913</td></tr>
              <tr><td>Year 8</td><td>7,275</td><td>4,331</td><td>4,331</td><td>15,937</td></tr>
              <tr><td>Year 9</td><td>7,376</td><td>4,331</td><td>4,331</td><td>16,038</td></tr>
            </tbody>
          </table>
        </div>

        <h5 className="section-title">Transport Fee</h5>
        <div className="table-container">
          <table className="custom-table text-center">
            <thead>
              <tr>
                <th>Route</th>
                <th>Term 1</th>
                <th>Term 2</th>
                <th>Term 3</th>
                <th>Annual Fee</th>
              </tr>
            </thead>
            <tbody>
              <tr><td>Sharjah</td><td>1,520</td><td>1,140</td><td>1,140</td><td>3,800</td></tr>
              <tr><td>Ajman</td><td>1,600</td><td>1,200</td><td>1,200</td><td>4,000</td></tr>
            </tbody>
          </table>
        </div>

        <div className="note-section">
          <p><strong>Note:</strong> The revised Tuition Fee and Transport Fee is effective from the academic year <strong>2024-2025</strong>,</p>
          <p>as per the approval of the Ministry of Education.</p>
          <p><em>* All fees are inclusive of VAT.</em></p>
        </div>
      </div>
    </div>
  );
}

export default SchoolFees;